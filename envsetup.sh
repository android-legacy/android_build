...
function cmremote()
{
    ...
    CMUSER=`git config --get review.review.cas-online.nl.username`
    if [ -z "$CMUSER" ]
    then
...
function cmgerrit() {
    ...
    local user=`git config --get review.review.cas-online.nl.username`
...
function cmrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "android-legacy Gerrit Rebase Usage: "
    ...
    git fetch "http://review.cas-online.nl/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
